...
//Multiplication
matrix matrix::operator*( matrix &mat) {
    if (this->columns != mat.rows){exit(9);}
...
double matrix::det(){
    if(this->rows != this -> columns){
        cout << "Can only calculate determinant on square matrices\n";
    }
    double result(0);
    if(this->rows == 1){
        result = (*this)(1,1);
    ...
    for(int i = 1; i<=this->columns; i++){
        if(i%2 == 1 ){
            result += (*this)(1,i) * (this->minor(1,i)).det();
        }
        else result -= (*this)(1,i) * (this->minor(1,i)).det();
    }
    return result;
}
